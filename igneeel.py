...
# coding=utf-8
# coding by SansayX
# Note : jangan cuma recode dek
# Open source code by SansayX | ngotak dikit cok jangan jual di perjual belikan...

## Jangan di jual belikan memek...
## Hargai gue dengan jangan ganti nama author...
## Dan jangan lupa suscribe chenel gue mmk...
## Buat yang mau ngamal ke no ini -> 085294383463
## Via DANA/OVO/GOPAY
...
def banner():
	print('''%s
 IGNEEL SIMPLE CRACK
[\x1b[1;96m+%s] IGNEEL SIMPLE CRACK
[\x1b[1;95m+%s] IGNEEL SIMPLE CRACK
───────────────────────────────────────────────────────
 [\x1b[1;94m+%s] Author   : SansayX
 [\x1b[1;95m+%s] Github   : https://github.com/SansayX
 [\x1b[1;97m+%s] Facebook : Ujaang Saani | facebook.com/ujang.sanibagja
───────────────────────────────────────────────────────\n'''%(N,N,N,N))
...
class menu:
	...
	def main(self):
		...
		jalan(' %s╔═[ %sHi, %s%s Jangan lupa makan:) ]'%(N,H,r,N))
		print(' %s║'%(N))
		print(' %s╠═[%s•%s] User License.     : %s'%(N,O,N,akss))
		print(' %s╠═[%s•%s] User IP                : %s'%(N,O,N,IP))
		print(' %s╠═[%s•%s] User Login         : %s'%(N,O,N,waktu))
		print(' %s║'%(N))
		...
		print(' %s╠═[%s7%s] Cek hasil crack.                 [ON]'%(N,O,N))
		print(' %s╠═[%s8%s] Setting User-Agent            [ON]'%(N,O,N))
...
class crack:
	...
	def free(self):
		...
		with SansayXD(max_workers=30) as dihi:
			for me in self.id:
				try:
					uid, name = me.split('<=>')
					sempak = name.split(' ')
